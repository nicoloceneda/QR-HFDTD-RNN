...
if args.debug:
    ...
    if not os.path.isdir('mode 2s bg'):

        os.mkdir('mode 2s bg')

    for symbol in symbol_list:

        if not os.path.isdir('mode 2s bg/' + symbol):

            os.mkdir('mode 2s bg/' + symbol)

        data = pd.DataFrame(output_resampled_f[output_resampled_f['sym_root'] == symbol][['date', 'time_m', 'price']])
        data.to_csv('mode 2s bg/' + symbol + '/data.csv', index=False)

else:

    if not os.path.isdir('mode 2s sl'):

        os.mkdir('mode 2s sl')

    for symbol in symbol_list:

        if not os.path.isdir('mode 2s sl/' + symbol):

            os.mkdir('mode 2s sl/' + symbol)

        data = pd.DataFrame(output_resampled_f[output_resampled_f['sym_root'] == symbol][['date', 'time_m', 'price']])
        data.to_csv('mode 2s sl/' + symbol + '/data.csv', index=False)
